...
public class KeyStoreExampleClientTest extends ScepServerSupport {
	...
	public void testExample() throws Exception {
		// For the sake of simplicity, we use an optimistic verifier.  This has no
		...
		if (caps.contains(Capability.SHA_1)) {
			...
			signerBuilder = new JcaContentSignerBuilder("SHA1withMD5");
		}
		...
		// Convert Bouncy Castle representation of X509Certificate into something usable
		...
		// The following variables are used to represent the entity being enrolled
		X500Name entityName = new X500Name("CN=entity");
		KeyPair entityPair = KeyPairGenerator.getInstance("RSA").genKeyPair();
		SubjectPublicKeyInfo publicKeyInfo = SubjectPublicKeyInfo.getInstance(entityPair.getPublic().getEncoded());
		...
		csrBuilder.addAttribute(PKCSObjectIdentifiers.pkcs_9_at_challengePassword, new DERPrintableString(new String("password".toCharArray())));
		...
		if (response.isFailure()) {
			...
			X500Principal entityPrincipal = new X500Principal(entityName.getEncoded());
			// We should deal with the response to the poll too.  Since this a short-lived
			// test, we conveniently stop processing here.  Usually you'd schedule the poll
			// to run at some point in the future.
			
			// It isn't a requirement to use the same ID and private key.
			response = client.poll(id, idPair.getPrivate(), entityPrincipal, response.getTransactionId());
			...
			entityStore.setKeyEntry("entity", entityPair.getPrivate(), "secret".toCharArray(), chain);
			entityStore.store(new FileOutputStream("store.jks"), "secret".toCharArray());
		}
	}
}
